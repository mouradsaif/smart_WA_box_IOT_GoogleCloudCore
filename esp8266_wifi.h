...
getlevelSensorValue();
return  "FullAvg: " + String(getlevelSensorValue()) ;
}
...
String getlevelSensorValue(){;
Serial.println("get-level");// send command for level module for send level sensor values 
serialEvent();//wait for level module send serial data 
Serial.println("get-level");// send command for level module for send level sensor values 
delay(100);//for level sensing module to reply
String DatafromSensor="";//level value string
if (stringComplete) {
    Serial.println(inputString);
    // clear the string:
    DatafromSensor=inputString;
    inputString = "";
